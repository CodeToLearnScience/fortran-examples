...
module fortran_dict
  ...
  ! Hash function using djb2 algorithm (simpler than FNV-1a and works with standard integers)
  function hash_function(this, key) result(hash_val)
    ...
    ! Use djb2 hash algorithm which doesn't require large constants
    hash_val = 5381  ! Initial value that fits in standard integer

    do i = 1, len(key)
      ! hash * 33 + c
      hash_val = hash_val * 33 + ichar(key(i:i))
    end do

    ! Handle potential overflow by taking modulo
    ...
    if (allocated(this%items)) then
      ...
      do i = 1, old_capacity
        if (this%items(i)%occupied) then
          ...
          if (allocated(this%items(i)%value)) then
  ...
  subroutine get_value(this, key, value, found)
    ...
    if (idx > 0 .and. this%items(idx)%occupied) then
  ...
  subroutine set_value(this, key, value)
    ...
    if (idx > 0) then
      ...
      if (this%items(idx)%occupied .and. allocated(this%items(idx)%key)) then
        if (this%items(idx)%key == key) then
  ...
  function get_all_keys(this) result(keys)
    ...
    do i = 1, this%capacity
      if (this%items(i)%occupied .and. allocated(this%items(i)%key)) then
        count = count + 1
...
program dict_demo
  ...
  type(dict) :: my_dict, my_int_dict 
  logical :: found, success
  integer :: i, int_val
  real :: real_val
  character(len=:), allocatable :: str_val
  character(len=256) :: int_to_str
  ...
  my_int_dict = create_dict(10)

  do i=1,10
    write(int_to_str, '(I10)') i 
    call my_int_dict%set(trim(adjustl(int_to_str)), 1)
  enddo 
  
  call my_int_dict%get("90", val, found) 
  
  if(found) then 
    print*, "found val"
  endif 

  !print*, my_int_dict%get_keys(), int_val 
